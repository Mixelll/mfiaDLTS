...
read_param_cell_s = read_param_cell(:,~contains(read_param_cell(1,:),sweep_order(2:3),'IgnoreCase',true));
read_param_cell_v = read_param_cell(:,contains(read_param_cell(1,:),sweep_order(2:3),'IgnoreCase',true));
sweep_pts = eval(['pts_' sweep_order{1}]);
l2 = eval(['pts_' sweep_order{2}]);
l3 = eval(['pts_' sweep_order{3}]);

for c = read_param_cell_v(3,:)
    tmp = nan(sweep_pts,l2*l3);
    for i = 1:size(tmp,2)
        tmp(:,i) = repmat(eval(['data_sweep_order_struct_vec(i)' c{:} '.''']), sweep_pts, 1);
    end
