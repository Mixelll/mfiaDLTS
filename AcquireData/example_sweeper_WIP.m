function data_3D = example_sweeper_WIP(device_id, device_properties, varargin)
...
p.addParamValue('start_osc_frequency', 1e3, @isnumeric);

stop = 500e3;
if strfind(props.devicetype, 'MF')
    stop = 10e6;
end
p.addParamValue('stop_osc_frequency', stop, @isnumeric);

p.addParamValue('start_test_signal', 0.05, @isnumeric);
p.addParamValue('stop_test_signal', 0.3, @isnumeric);

p.addParamValue('start_offset', 1, @isnumeric);
p.addParamValue('stop_offset', -1, @isnumeric);
...
p.addParamValue('demod_timeconstant', 0.007, @isnumeric);

% demod rate, [Hz].
p.addParamValue('demod_rate', 0.1, @isnumeric);
...
ziDAQ('set', h, 'samplecount', p.Results.pts_osc_frequency);
...
k = 0;
j = 0;
for val_k = linspace(eval(['p.Results.start_' sweep_order{3}]), eval(['p.Results.stop_' sweep_order{3}]), eval(['p.Results.pts_' sweep_order{3}]))
	ziDAQ('setDouble', ['/' device 'imps/0/' dict1(sweep_order{3}) '/' dict2(sweep_order{2})], val_k);
    k = k + 1;
    eval(['p.Results.pts_' sweep_order{3}])
	for val_j = linspace(eval(['p.Results.start_' sweep_order{2}]), eval(['p.Results.stop_' sweep_order{2}]), eval(['p.Results.pts_' sweep_order{2}]))
        ziDAQ('setDouble', ['/' device 'imps/0/' dict1(sweep_order{2}) '/' dict2(sweep_order{2})], val_j);
        j = j + 1;
...
data = [];
pts = eval(['p.Results.pts_' sweep_order{1}]);
sweep_i = nan(1, pts);
r = nan(1, pts);
theta = nan(1, pts);
p0 = nan(1, pts);
...
% i = '(:, f, a)=';
% elseif strcamp(sweep_by, 'osc_frequency')
% i = '(v, :, a)=';
% elseif strcamp(sweep_by, 'test_signal')
% i = '(v, f, :)=';
% end
%
eval([sweep_order{1} '=sweep_i']);
eval([sweep_order{2} '=sweep_j']);
eval([sweep_order{3} '=sweep_k']); 
for s = {'r', 'theta', 'param0', 'param1', 'bias_voltage', 'osc_frequency', 'test_signal'} 
    eval([s '_3D(i, j, :)' s]) 
end
